...
class Login:
    def crearLogin(sel, objPeople):
        
        def ejecutaValidacion():
            status = objPeople.validarUsuario(var1.get(), var2.get())
            if (status):
                print(messagebox.showinfo('Bienvenido', 'Acceso Concedido'))
            else:
                print(messagebox.showerror('Error','Usuarion no encontrado'))
        Ventana = Tk()
        ...
        Label (seccion1, text="Logion FPOO", bg="black", fg = "white", font =("Mono",18)).pack()
